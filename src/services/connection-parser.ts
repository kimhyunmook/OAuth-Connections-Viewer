    ...
    try {
        ...
        if (!parentElement) {
            ...
            if (retryCount < maxRetries) {
                console.log(`Parent element not found for ${parentDom}, retrying in 5 seconds... (attempt ${retryCount + 1}/${maxRetries + 1})`);
                setTimeout(() => {
                    parseConnection(
                ...
                console.warn(`Failed to find parent element after ${maxRetries + 1} attempts: ${parentDom}`);
                return [];
            }
        }

        const items = Array.from(parentElement.querySelectorAll(listDom)).map((list) => {
        ...
        () => {
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    if (retryCount < maxRetries) {
                        console.log(`No data found for ${type}, retrying in 5 seconds... (attempt ${retryCount + 1}/${maxRetries + 1})`);
                        sendMessageData(type, dataFunction, 5000, retryCount + 1, maxRetries);
                    } else {
                        console.warn(`No data found after ${maxRetries + 1} attempts for ${type}`);
                    }
                }
            } catch (error) {
